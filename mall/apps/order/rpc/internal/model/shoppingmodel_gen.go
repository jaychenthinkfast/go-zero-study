...
var (
	...
	shoppingRowsExpectAutoSet   = strings.Join(stringx.Remove(shoppingFieldNames, "`id`", "`create_at`", "`create_time`", "`created_at`", "`update_at`", "`update_time`", "`updated_at`"), ",")
...
type (
	shoppingModel interface {
		...
		FindOne(ctx context.Context, id uint64) (*Shopping, error)
...
func newShoppingModel(conn sqlx.SqlConn, c cache.CacheConf, opts ...cache.Option) *defaultShoppingModel {
...
func (m *defaultShoppingModel) Delete(ctx context.Context, id uint64) error {
...
func (m *defaultShoppingModel) FindOne(ctx context.Context, id uint64) (*Shopping, error) {
	ordersShoppingIdKey := fmt.Sprintf("%s%v", cacheOrdersShoppingIdPrefix, id)
	var resp Shopping
	err := m.QueryRowCtx(ctx, &resp, ordersShoppingIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultShoppingModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheOrdersShoppingIdPrefix, primary)
}

func (m *defaultShoppingModel) queryPrimary(ctx context.Context, conn sqlx.SqlConn, v, primary any) error {
	query := fmt.Sprintf("select %s from %s where `id` = ? limit 1", shoppingRows, m.table)
